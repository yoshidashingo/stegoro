...
生成されたユーザーハーネスは、**Claude Codeエージェントスキル**としてパッケージ化できます。これにより、配布・インストール可能なプラグインになります。

生成されたポリシーファイルに `.claude-plugin/` メタデータディレクトリを追加するだけで、ハーネスはプラグインに変わります。インストールしたユーザーは手動のファイルセットアップなしにスラッシュコマンドでエージェントを呼び出せます。

```bash
# 公開済みエージェントスキルをインストール
...
このデプロイモデルは [`samples/`](samples/) ディレクトリのサンプルエージェントで採用されています。各サンプルには `.claude-plugin/` ディレクトリが含まれており、Claude Codeマーケットプレイスへの公開に対応しています。
